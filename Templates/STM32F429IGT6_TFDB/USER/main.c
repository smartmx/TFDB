...
tfdb_addr_t addr = 0, addr1 = 0; /*addr cache*/
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (key == KEY0_PRES) //KEY0按下,读取字符串并显示
        {
            ...
            result = tfdb_get(&test_index, (uint8_t *)test_buf, &addr1, &test_value);
            if (result == TFDB_NO_ERR)
            {
                printf("get ok, addr:%x\n", addr1);
								addr1 = 0;
						...
						{
								LCD_ShowString(30, 170, 200, 16, 16, "Data Readed Error"); //提示传送完成
								LCD_ShowString(30, 190, 200, 16, 16, test_value); //显示读到的字符串
						}
        }
        i++;
